...
export default function VocabularyConfirmModal({ children, onConfirm, onClose }: Props) {
  ...
  return (
    <dialog
      ...
      className="fixed inset-0 m-auto w-[calc(100%-44px)] max-w-75.5 rounded-xl border-0 bg-[#F7F6F1] px-7 pt-9 pb-2.5 text-black shadow-[0_5px_10px_#0003] backdrop:bg-[#302a2440]"
    >
      <p
      ...
      <div className="mt-2.5 flex justify-end gap-2">
        <button
          type="button"
          onClick={onConfirm}
          className="h-6 min-w-10 rounded bg-[#F7F6F1] px-3 text-sm text-black active:bg-[#D91414] active:text-white"
        >
          예
        </button>
        <button
